...
const findOne = async (req, res) => {
  ...
  const rows = await query(
    `SELECT * FROM journals WHERE user_id = '${req.params.id}'`,
  );
...
const createOne = async (req, res) => {
  ...
  const rows = await query(
    `INSERT INTO journals (title, content, date, user_id) VALUES ('${req.body.title}', '${req.body.content}', '${req.body.date}', '${req.body.user_id}')`,
  );
